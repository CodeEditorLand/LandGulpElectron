			...
			es.map(function (f, cb) {
				...
				f.contents.on("end", function () {
					...
					if (opts.darwinHelpBookFolder && opts.darwinHelpBookName) {
						infoPlist["CFBundleHelpBookFolder"] =
							opts.darwinHelpBookFolder;
						infoPlist["CFBundleHelpBookName"] =
							opts.darwinHelpBookName;
					}
						...
						infoPlist["CFBundleDocumentTypes"] = (
							...
							opts.darwinBundleDocumentTypes.map(function (type) {
								...
								var result = {
									...
									CFBundleTypeIconFile: path.basename(
										type.iconFile,
									),
								};
					...
					if (opts.darwinBundleURLTypes) {
						infoPlist["CFBundleURLTypes"] =
							opts.darwinBundleURLTypes.map(function (type) {
								return {
									CFBundleTypeRole: type.role,
...
function renameAppHelper(opts) {
	...
	return rename(function (path) {
		...
		if (
			...
			var isInMacOS =
				/Contents\/Frameworks\/Electron\ Helper( \(\w+\))?\.app\/Contents\/MacOS$/.test(
					path.dirname,
				);
