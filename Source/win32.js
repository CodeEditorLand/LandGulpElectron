...
function getSignTool() {
	...
	let latestWindowsSDKVersion = signToolPaths
		.map((x) => x.replace(windowsSDKDir, "").split("\\")[0])
		.pop();
	let latestWindowsSDKSignTools = signToolPaths.filter((x) =>
		x.startsWith(`${windowsSDKDir}${latestWindowsSDKVersion}`),
	);
...
function patchExecutable(opts) {
	return es.map(function (f, cb) {
		...
		ostream.on("close", function () {
			...
			const { error } = spawnSync(signToolPath, [
				"remove",
				"/s",
				tempPath,
			]);
